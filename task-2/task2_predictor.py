...
def main():
    ...
    df = pd.read_csv("to_be_predicted_test.tsv", sep="\t", header=None, names=["sent","article_id","sent_id"])
...
#    df = None
    df2 = pd.DataFrame(columns=["article_id", "sent_id", "prediction"])
...
#        with torch.no_grad():
        ...
        df2 = df2.append([{"article_id":df.iloc[int(doc_ids[i].item())].article_id, "sent_id":df.iloc[int(doc_ids[i].item())].sent_id, "prediction":labels[i]} for i in range(len(labels))], ignore_index=True)
#        logger.info("Done 1 batch")

    df2 = df2.apply(to_out, axis=1)

    # We discarded empty lines earlier for the classification, in here we make every empty line non-propaganda
    all_df = pd.read_csv("all_test.tsv", sep="\t", header=None, names=["sent","article_id","sent_id"])
    all_df["label"] = "non-propaganda" # This takes care of empty lines
    for i,row in all_df.iterrows():
        # Need this try because some of the sentences are not classified by BERT
        try:
            all_df.label.iloc[i] = df2[(df2.article_id == row.article_id) & (df2.sent_id == row.sent_id)].prediction.iloc[0]
        except:
            pass

    all_df = all_df.drop(columns=["sent"])
    all_df.to_csv("predictions_test.tsv", sep="\t", index=False, header=None)
